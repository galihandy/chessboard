...
public class CustomAdapter extends BaseAdapter implements Filterable {
	...
	private void bindView(int position, View view) {
		...
		for (int i = 0; i < count; i++) {
			...
			if (v != null) {
				...
				String text = null;
				Bitmap image = null;
				// disini data diubah jadi text
				if (data instanceof Bitmap) {
					image = (Bitmap) data;
					// Log.i("IMAGE VIEW", "MASUK SINI");
				} else {
				...
				if (!bound) {
					if (v instanceof Checkable) {
						...
						if (data instanceof Integer) {
							...
							setViewImage((ImageView) v, image);
						}
					} else {
	...
	private class SimpleFilter extends Filter {
		...
		protected void publishResults(CharSequence constraint,
			...
			mData = (List<HashMap<String, Object>>) results.values;
			if (results.count > 0) {
	...
	public void addElement(ArrayList<Event> arg1, int cat, ArrayList<Event> arc) {
		for (int i = 0; i < arg1.size(); i++) {
			HashMap<String, Object> map = new HashMap<String, Object>();
			map.put(Database.EVENT_NAME, arg1.get(i).getName());
			map.put(Database.EVENT_START_DATE, arg1.get(i).getStartDate());
			map.put("image", arg1.get(i).getImage());
			mData.add(map);
		}
		notifyDataSetChanged();
	}
